...
def create_two_sounds(fs, f1, r1, theta1, f2, r2, theta2):
    ...
    x = r1 * np.cos(2 * np.pi * f1 * t - theta1) + r2 * np.cos(
        2 * np.pi * f2 * t - theta2
    )
    return t, x


def estimate_a_b(fs, t, x, n):
    # a,bの計算
...
if __name__ == "__main__":
    ...
    t, x = create_two_sounds(fs, n1, r1, theta1, n2, r2, theta2)
    amplitude, phase = estimate_a_b(fs, t, x, n1)

    print(f"amplitude {n1}Hz : {amplitude}")
    print(f"phase {n1}Hz : {phase}")

    amplitude, phase = estimate_a_b(fs, t, x, n2)
